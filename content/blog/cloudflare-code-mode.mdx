...
My workaround has been manual curation: disable servers I don't need right now, keep the active set small, reconnect when the task changes. It works, but it is overhead that should not exist. Code Mode promises a world where that curation is unnecessary, and an agent can have access to everything and only load what the current task requires.
...
The MCP ecosystem is barely a year old and already [hitting scaling walls that nobody anticipated](/blog/mcp-outgrew-design). Code Mode is the first credible answer. The teams that adopt it early get access to API surfaces that remain impractical for everyone else, and in a landscape where integration breadth determines agent capability, that is a real advantage.

**P.S.** If you are wiring MCP into an agent and the tool surface is getting out of hand, I have been building around these limits: fewer tools, code instead of schemas, and the sandbox rules that make it safe. If your tool surface needs that treatment, DM me on [X](https://x.com/vvegalex) and tell me what you are building.
